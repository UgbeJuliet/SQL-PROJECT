...
--3.display in descending order of senority the male employees whose net salary
--(salary + commission) is greater than or equal to 8000
--The resulting table should include the following columns: employee number, last name, first name, 
--age and senority

SELECT EMPLOYEE_int AS EmployeeNumber ,
...
--4.display products that meet the following criteria:(C1) quantity is packaged in bottles
--(C2) the third character in the product name is 't' or 'T',(C3)
--Supplied by suppliers 1,2,3 (C4) the unit price is between 70 and 200 and
--(C5) the number of units are specified (not null). The resulting table should include
-- the following columns: product number, product name, supplier number, 
--units ordered and unit price.


SELECT PRODUCT_REF AS 'PRODUCT NUMBER',PRODUCT_NAME, SUPPLIER_int AS 'SUPPLIER NUMBER',
UNITS_ON_ORDER AS 'Units Ordered' ,UNIT_PRICE AS 'Unit Price'
FROM dbo.PRODUCTS
WHERE QUANTITY LIKE '%bottles%' 
   AND (PRODUCT_NAME LIKE '__t' OR PRODUCT_NAME LIKE '__T')
   AND SUPPLIER_int = 1 OR SUPPLIER_int = 2 OR SUPPLIER_int = 3
...
CASE 
    WHEN SUM(UNIT_PRICE * QUANTITY) BETWEEN 0 AND 2000 THEN '0.00'
    WHEN SUM(UNIT_PRICE * QUANTITY) BETWEEN 2001 AND 100 THEN '0.05'
    WHEN SUM(UNIT_PRICE * QUANTITY) BETWEEN 10001 AND 40000 THEN '0.10'
    WHEN SUM(UNIT_PRICE * QUANTITY) BETWEEN 40001 AND 80000 THEN '0.15'
    ELSE '0.20'
    ...
    --7.Display suppliers of beverage products. 
    --The resulting table should display the columns: 
    --supplier number, company, address, and phone number.
    SELECT DISTINCT sp.SUPPLIER_int AS 'Supplier Number', COMPANY, ADDRESS, PHONE AS 'Phone Number'
    FROM dbo.SUPPLIERS AS sp
    INNER JOIN dbo.PRODUCTS AS p ON sp.SUPPLIER_int = p.SUPPLIER_int
    INNER JOIN dbo.CATEGORIES AS c ON p.CATEGORY_CODE = c.CATEGORY_CODE
...
FROM dbo.ORDERS o
LEFT JOIN dbo.ORDER_DETAILS od ON o.ORDER_int = od.ORDER_int
LEFT JOIN dbo.PRODUCTS p ON od.PRODUCT_REF = p.PRODUCT_REF
LEFT JOIN dbo.CATEGORIES cat ON p.CATEGORY_CODE = cat.CATEGORY_CODE
WHERE o.CUSTOMER_CODE = CUSTOMERS.CUSTOMER_CODE
AND cat.CATEGORY_NAME = 'Desserts') <= 1;
 ...
 SELECT c.Customer_Code AS 'Customer Number', Company AS 'Company Name',
Phone AS 'Phone Number', COALESCE(SUM(Quantity * Unit_Price * (1- Discount)), 0) AS 'Total Amount',
Country
FROM dbo.Customers c
LEFT JOIN dbo.Orders o ON c.Customer_Code = o.Customer_Code
LEFT JOIN dbo.Order_Details od ON o.Order_int = od.Order_int
AND DATEPART(WEEKDAY, o.ORDER_DATE) = 2  -- DATEPART(DAY, o.Order_Date)
...
--10.--Display customers who have ordered all products. The resulting table should display the columns: customer code, 
...
WHERE NOT EXISTS (
    ...
    WHERE NOT EXISTS (
        SELECT 1
        FROM dbo.ORDERS o
        INNER JOIN dbo.ORDER_DETAILS od ON o.ORDER_int = od.ORDER_int
